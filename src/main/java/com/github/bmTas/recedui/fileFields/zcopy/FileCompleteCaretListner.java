		...
		if (filesListDialog == null) {
			filesListDialog = new UiFileSelectionList(
					...
					new IListPopupActions<Path>() {
						...
						@Override public void fireFileChanged(Path f) {
							setSelectedPath(f.toFile());
						}
					});
						...
						if (filesListDialog.getSelectedIndex() != -1) {
							if (e.getModifiers() == KeyEvent.CTRL_MASK) {
								...
								case KeyEvent.VK_ENTER:
									setSelectedPath(filesListDialog.getSelectedValue().toFile());
									break;
								case KeyEvent.VK_UP:
									int idx1 = listSize - 1;
		...
		if (! skip) {
			SwingUtilities.invokeLater(new Runnable() {
				@Override public void run() {
					filesListDialog.updateFileList(fileList);
